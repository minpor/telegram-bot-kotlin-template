...
) : BotCommand(CommandCode.CONTACT.command, CommandCode.CONTACT.desc) {


    override fun execute(telegramClient: TelegramClient, user: User, chat: Chat, arguments: Array<out String>) {
        val chatId = chat.id

        usersRepository.updateUserStep(chatId, StepCode.CONTACT)

        applicationEventPublisher.publishEvent(
            TelegramStepMessageEvent(chatId = chatId, stepCode = StepCode.CONTACT)
        )
    }
}